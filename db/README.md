...
Both migrations are applied to staging and production; the two are kept in step so production never carries tenant tables without policies. Production does not have the pgTAP extension (a test-only tool).
